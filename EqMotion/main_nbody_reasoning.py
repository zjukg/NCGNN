...
parser.add_argument("--ncgnn",action='store_true', default=True)
...
def main():
    ...
    model = EqMotion(in_node_nf=args.past_length, in_edge_nf=2, hidden_nf=args.nf, in_channel=args.past_length, hid_channel=64, out_channel=args.future_length,device=device, n_layers=args.n_layers, recurrent=True, norm_diff=args.norm_diff, tanh=args.tanh, ncgnn=args.ncgnn)    
...
def train(model, optimizer, epoch, loader, backprop=True):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        loc_pred, category, vel_pred = model(nodes, loc.detach(), vel.detach(),edge_attr)
...
def test(model, optimizer, epoch, loader, backprop=True):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(loader):
            ...
            vel = vel * constant
            nodes = torch.sqrt(torch.sum(vel ** 2, dim=-1)).detach()
            loc_pred, category_list, vel = model(nodes, loc.detach(), vel.detach(),edge_attr)

            if validate_reasoning:
                for l in range(0,len(category_list)):
                    # print(evaluate_accuracy(category_list[l],edge_attr,log=(batch_idx==0 and l == 0)))
