...
class EqMotion(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, in_channel, hid_channel, out_channel, device='cpu', act_fn=nn.SiLU(), n_layers=4, coords_weight=1.0, recurrent=False, norm_diff=False, tanh=False, ncgnn=False):
    ...
    def newton_cotes_rules(self, n_step):
        ...
        # weights = na * np.array(vi, dtype=float) / da
        weights = np.array(vi, dtype=float) #/ sum(vi)
        error_coeff = nb / db
    ...
    def forward(self, h, x, vel, edge_attr=None):
        ...
        if self.ncgnn:
            xs, vs = [], []
            for step in range(nc_k):
                for i in range(0, self.n_layers):
                    h, x, _, vel = self._modules["gcl_%d" % i](h, x, vel, edge_attr=edge_attr, category=category)
                    cagegory_per_layer.append(category)
                xs.append(torch.clone(x))
                vs.append(torch.clone(vel))
                # vel = cood_vel

            weights, error_coeff = self.newton_cotes_rules(nc_k)
            xs, vs = torch.stack(xs, 1), torch.stack(vs, 1)
            # print(x.shape, xs.shape, weights.reshape([-1, 1]).shape)
            quad = torch.sum(vs * weights.reshape([1, -1]), 1) / weights.sum()
            # print(vs.shape, quad.shape)
            x = xs[:, 0] + quad
            vel = vs[:, -1]
        else:
            for i in range(0, self.n_layers):
                h, x, _, __ = self._modules["gcl_%d" % i](h, x, vel, edge_attr=edge_attr, category=category)
                cagegory_per_layer.append(category)

        x_mean = torch.mean(torch.mean(x,dim=-2,keepdim=True),dim=-3,keepdim=True)
        
        x = self.predict_head((x-x_mean).transpose(2,3)).transpose(2,3) + x_mean
        vel = self.predict_head(vel.transpose(2,3)).transpose(2,3)
        # print(x.shape, vel.shape)
        if self.apply_dct:
            x = torch.matmul(idct_m,x)
            x = x + x_center
            vel = torch.matmul(idct_m,vel)
        if self.validate_reasoning:
            return x, cagegory_per_layer, vel
        else:
            return x, None, vel
