...
class MD17Dataset():
    ...
    def __init__(self, partition, max_samples, delta_frame, data_dir, molecule_type):
        ...
        full_dir = join(data_dir, molecule_type + '_dft.npz')
        split_dir = join(data_dir, molecule_type + '_split.pkl')
...
class MD17SeqDataset():
    """
    MD17 Dataset

    """
    # step
    def __init__(self, partition, max_samples, delta_frame, data_dir, molecule_type, n_step=2):
        # setup a split, tentative setting
        train_par, val_par, test_par = 0.1, 0.05, 0.05
        full_dir = join(data_dir, molecule_type + '_dft.npz')
        split_dir = join(data_dir, molecule_type + '_split.pkl')
        ...
        assert delta_frame % n_step == 0
        step_size = int(delta_frame//n_step) 

        # print(x.shape, st.shape, delta_frame, step_size)
        st_ext = np.repeat(st[:, np.newaxis], n_step+1, 1)  # (N, n_step+1)
        steps = np.arange(delta_frame+1)[0:delta_frame+1:step_size] # (n_step+1,) 
        seq_indices = st_ext + steps.reshape((1, -1)) # (N, n_step+1)
        x_seq = x[seq_indices]
        v_seq = v[seq_indices]
        # print(seq_indices[0], x_seq.shape, v_seq.shape)
        print('Got {:d} samples!'.format(x_0.shape[0]))
        # exit()
        mole_idx = z
        n_node = mole_idx.shape[0]
        ...
        self.x_0, self.v_0, self.x_t, self.v_t = torch.Tensor(x_0), torch.Tensor(v_0), torch.Tensor(x_t), torch.Tensor(
            v_t)
        self.x_seq, self.v_seq = torch.Tensor(x_seq).transpose(1,2), torch.Tensor(v_seq).transpose(1,2) # (N, num_node, num_step, 3)
        self.mole_idx = torch.Tensor(mole_idx)
    ...
    def __getitem__(self, i):
        ...
        return self.x_seq[i], self.v_seq[i], self.x_0[i], self.v_0[i], edge_attr, self.mole_idx.unsqueeze(-1), self.x_t[i], self.v_t[i], self.Z.unsqueeze(-1), cfg
    ...
    def get_edges(self, batch_size, n_nodes):
        ...
        edges = [torch.LongTensor(self.edges[0]), torch.LongTensor(self.edges[1])]
        # print(edges[0].shape, batch_size, n_nodes)
        if batch_size == 1:
            return edges
        elif batch_size > 1:
            rows, cols = [], []
