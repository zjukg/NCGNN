...
class EGNN_vel(nn.Module):
    ...
    def forward(self, h, x, edges, vel, edge_attr):
        h = self.embedding(h)
        for i in range(0, self.n_layers):
            h, x, vel = self._modules["gcl_%d" % i](h, edges, x, vel, edge_attr=edge_attr)
        return x, vel
...
class DMD(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=4, coords_weight=1.0,
        ...
        for i in range(n_layers):
            self.add_module("gcl_%d" % i, DMDLayer(self.hidden_nf, self.hidden_nf, self.hidden_nf,
...
class RF_vel(nn.Module):
    ...
    def forward(self, vel_norm, x, edges, vel, edge_attr):
        for i in range(0, self.n_layers):
            x, vel = self._modules["gcl_%d" % i](x, vel_norm, vel, edges, edge_attr)
        return x, vel
